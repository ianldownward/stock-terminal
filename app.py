...
class PortfolioManager:
    ...
    def _ensure_default_user(self):
        try:
            ...
            # --- SEARCH AND RESCUE MIGRATION ---
            # This looks at all fragmented profiles and pulls the one with your actual history
            recovery_mapping = {
                'Test': ['Test A - Deep Value', 'Test'],
                'Test 2': ['Test B - US Momentum', 'Test B - Momentum (UK & US)', 'Test 2'],
                'Test 3': ['Test C - 24/5 Global', 'Test 3'],
                'Test 4 - Volatility Breakout': ['Test D - Volatility', 'Test 4 - Volatility Breakout'],
                'Test 5 - Rel Strength Rotator': ['Test E - Rotator', 'Test 5 - Rel Strength Rotator'],
                'Ian': ['Ian']
            }
            ...
            for target_profile, possible_names in recovery_mapping.items():
                best_data = self.default_user_state(target_profile)
                max_h = -1
                for name in possible_names:
                    if name in self.data['users']:
                        h_len = len(self.data['users'][name].get('history', []))
                        if h_len > max_h:
                            max_h = h_len
                            best_data = self.data['users'][name]
                recovered_users[target_profile] = best_data

            if list(self.data['users'].keys()) != list(recovered_users.keys()):
                needs_save = True

            self.data['users'] = recovered_users

            if self.data.get('active_user') not in self.data['users']:
                self.data['active_user'] = 'Test 2'
                needs_save = True
