...
class PortfolioManager:
    ...
    def _ensure_default_user(self):
        try:
            ...
            # This instantly pulls your data back from the renamed profiles
            revert_map = {
                'Test A - Deep Value': 'Test',
                'Test B - Momentum (UK & US)': 'Test 2',
                'Test B - US Momentum': 'Test 2',
                'Test C - 24/5 Global': 'Test 3',
                'Test D - Volatility': 'Test 4 - Volatility Breakout',
                'Test E - Rotator': 'Test 5 - Rel Strength Rotator',
                'Test F - EOD Sweep': 'Test 6 - EOD Cash Sweep'
            }
            ...
            for new_name, old_name in revert_map.items():
                if new_name in self.data.get('users', {}):
                    self.data['users'][old_name] = self.data['users'].pop(new_name)
                    if self.data.get('active_user') == new_name:
                        self.data['active_user'] = old_name
                    needs_save = True

            # --- STANDARD PRE-TEST 6 LOGIC ---
